...
**问题**：数百用户同时出价，如果用传统"读取当前价 → 比较 → 写入"的流程，会出现经典的并发竞态 — 两个用户同时读到相同的当前价，都判定自己出价有效，导致低价覆盖高价，产生资金纠纷。加锁又会严重拖慢吞吐量。

**解决方案 — Redis Lua 原子出价 + 五层漏斗**：

将出价的"校验 → 更新 → 排行榜 → 延时判断"全部封装进一段 Redis Lua 脚本，单次 `EVAL` 调用原子完成，消除竞态窗口。在 Lua 之前叠加四层前置过滤，逐层削减无效请求：

```
请求量   100%  ──→  L1 内存预校验（缓存当前价/状态，过滤明显无效）
         ↓ ~40%
              ──→  L2 去重（同用户同金额 500ms 内去重，防手抖连击）
         ↓ ~35%
              ──→  L3 Redis 限流（每人每秒 1 次，SetNX + TTL）
         ↓ ~30%
              ──→  L4 Redis Lua 原子出价（校验+更新+延时，一次调用）
         ↓
              ──→  L5 异步持久化（channel 缓冲 → 批量写入 MySQL）
```

**效果**：实测 2000 并发用户 1,357 QPS，64,991 次出价零数据错误。L1~L3 三层过滤掉 60-70% 的无效请求，大幅降低 Redis 压力。

### 难点二：跨端毫秒级倒计时同步

**问题**：竞拍倒计时是最敏感的 UI 元素 — 用户根据剩余时间决定是否出价。但客户端时钟和服务端时钟存在偏差（可达数秒），加上网络延迟不确定，如果简单用 `Date.now()` 计算剩余时间，不同用户看到的倒计时会不一致。`setInterval` 在浏览器后台标签页还会被节流，导致倒计时"跳秒"。

**解决方案 — NTP 风格时钟校准 + requestAnimationFrame**：

1. **时钟校准**：客户端发送 `sync_time` 消息，记录发送时间 `t1`，服务端返回服务端时间 `ts`，客户端收到时记录 `t2`，计算偏差 `offset = ts - (t1 + t2) / 2`。多次采样取中位数，过滤网络抖动
2. **权威时间源**：服务端每 5 秒推送 `countdown_sync` 消息携带权威结束时间，客户端据此修正倒计时
3. **渲染层**：使用 `requestAnimationFrame` 而非 `setInterval` 驱动倒计时渲染，保证 60fps 流畅更新，最后 10 秒切换为红色脉冲动画营造紧张感

**效果**：全端倒计时偏差控制在 ±50ms 以内，浏览器后台标签页切回后也能立即校准，无"跳秒"或"时间倒流"现象。

### 难点三：盲拍模式下的数据隔离与防泄露

**问题**：盲拍模式要求用户只能看到自己的出价金额和排名变动，看不到他人的具体金额。但 WebSocket 广播天然是"一对多"的 — 如果像明拍一样把完整数据广播给所有人，哪怕前端隐藏了金额，用户只要打开浏览器 DevTools 就能看到所有人的出价。HTTP 接口也存在同样的泄露风险。

**解决方案 — 服务端逐用户消息定制 + HTTP 同步脱敏**：

1. **WebSocket 层**：`BroadcastService` 在盲拍模式下不做统一广播，而是遍历房间内每个客户端，为每人生成定制消息 — 只有 `isMe: true` 的出价行携带金额，其余显示 `amount: null`（前端渲染为 `¥ • • • •`）
2. **HTTP 层**：排行榜、竞拍详情等 REST 接口同步应用相同的脱敏逻辑，根据请求者 `userID` 过滤金额字段
3. **虚拟昵称**：每位用户进入竞拍时通过 `AliasService` 分配固定虚拟昵称（如"巅峰竞拍者3"），整场不变，跨场重新分配，彻底隔离真实身份
4. **揭晓机制**：竞拍结束后自动"开奖"，揭晓全部出价金额和排名

**效果**：即使用户抓包或调试 WebSocket 帧，也只能看到自己的金额和他人的排名序号，无法获取任何敏感竞价信息。

## 项目亮点

### 1. 全链路原子化出价引擎 — 从内存到 Redis 的五层漏斗

区别于常见竞拍系统"后端接收 → 数据库锁行 → 更新"的串行流程，本系统在出价请求到达 Redis 之前已经通过内存预校验和去重过滤掉 60-70% 的无效请求，剩余请求由 Redis Lua 脚本在单次 `EVAL` 中原子完成"价格校验 → 状态更新 → 排行榜更新 → 延时判断"四步操作，最终异步批量持久化到 MySQL。整条链路不使用任何分布式锁，却保证了严格的数据一致性。实测峰值 **1,357 QPS / 0% 错误率**。

### 2. 事件驱动竞拍状态机 — 6 状态 × 10 种转移 × 自动副作用

竞拍生命周期用 FSM（有限状态机）建模：`DRAFT → PENDING → ACTIVE → EXTENDED → COMPLETED / FAILED / CANCELLED`。每次状态转移自动触发副作用链 — 进入 `ACTIVE` 时启动 Timer + 初始化 Redis + 广播开始；进入 `EXTENDED` 时执行延时衰减策略（30s → 25s → ... → 5s，最多 10 次 + 总时长不超过 3 倍原定时长）；进入 `COMPLETED` 时自动从 Redis 获取赢家、生成订单、广播成交。所有状态流转都有 Guard 条件保护，杜绝非法转移。

### 3. 全栈可观测的生产级工程质量

从 Prometheus 20+ 自定义指标（覆盖 HTTP/WebSocket/Bid Pipeline/Redis 命令级延迟）到内置的并发压测工具（一键完成注册→建房→开拍→出价→报告），再到 Docker Compose 五服务编排（MySQL + Redis + Backend + Frontend + Prometheus），整个项目按生产标准而非 Demo 标准构建。压测报告覆盖 10 ~ 10,000 并发用户共 7 个梯度，包含 QPS、延迟分位数（P50/P90/P95/P99）、连接成功率、错误分布等完整数据，可直接用于技术答辩。
