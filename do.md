...
1. **用户注册/登录** → JWT 鉴权 → Token 持久化
2. **商家创建商品** → 上架（DRAFT → LISTED）→ 创建直播间 → 开播
...
| 接口 | 功能 |
|------|------|
| `POST /api/auth/register` | 用户/商家注册（手机号+密码+角色） |
...
|------|------|
| 登录/注册页 | 手机号密码登录 |
| 首页（直播间列表） | 展示所有直播中房间，底部 Tab 导航 |
| 竞拍房间页 | 直播流播放、实时倒计时、排行榜、保证金缴纳、出价面板、通知动画 |
| 我的竞拍 | 出价历史（按场次分组，最高出价，中标标识） |
| 我的订单 | 订单列表 + 详情（支付/确认收货/取消） |

### 核心组件
- **LiveStreamPlayer** — 视频流播放（支持 streamUrl / demo 回退），LIVE 红标、观看人数叠加层
- **CountdownTimer** — requestAnimationFrame 毫秒级渲染，最后10秒红色脉冲
- **RankingList** — 排名徽章，高亮自己，盲拍隐藏他人金额
- **BidController** — 保证金缴纳入口 + 出价输入 + 快捷加价 + loading态防重复
...
| 页面 | 功能 |
|------|------|
| 登录页 | 商家登录 + MERCHANT 角色校验 |
| 仪表盘 | 6 项统计数据（直播中/进行中竞拍/已成交/商品数/订单数/累计营收） |
...
| Demo 回退 | 无 streamUrl 时自动播放 demo 视频 |
| UI 叠加层 | LIVE 红标、房间标题、观看人数、连接状态 |
| 加载失败回退 | 视频加载失败时展示动画占位 + "直播画面加载中" |
...
| 组件 | 状态 |
|------|------|
| Docker Compose 一键部署（MySQL + Redis + Backend + Frontend + Prometheus） | ✅ |
| 后端多阶段 Dockerfile（~20MB 镜像） | ✅ |
| 前端 Dockerfile（pnpm build + Nginx） | ✅ |
| Nginx 反向代理（API + WebSocket + SPA 路由） | ✅ |
| 配置文件 + 环境变量覆盖 | ✅ |
...
| 表 | 用途 |
|----|------|
| `users` | 用户/商家统一表（手机号、昵称、bcrypt密码、角色） |
